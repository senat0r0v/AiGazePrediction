...
public class IOSGazeController : MonoBehaviour
{
    ...
    public int gridWidth = 3;  // Width of the grid (x-axis) for 3x3 grid
    public int gridHeight = 3; // Height of the grid (z-axis) for 3x3 grid
    ...
    void InitializeRegionColors()
    {
        ...
        regionColors.Add("Down-Left", Color.white);
        regionColors.Add("Left", Color.white);
        regionColors.Add("Up-Left", Color.white);
        regionColors.Add("Down", Color.white);
        regionColors.Add("Center", Color.white);
        regionColors.Add("Up", Color.white);
        regionColors.Add("Down-Right", Color.white);
        regionColors.Add("Right", Color.white);
        regionColors.Add("Up-Right", Color.white);
    }
    ...
    void GenerateGrid()
    {
        ...
        for (int x = -1; x <= 1; x++)
        {
            for (int z = -1; z <= 1; z++)
            {
                ...
                GameObject cube = Instantiate(cubePrefab, new Vector3(x * 2f, 0, z * 2f), Quaternion.identity);
    ...
    {
        // Map grid positions to classes
        // (x, z) ranges from (-1,-1) to (1,1)
        // Each combination corresponds to a class
        if (x == -1 && z == -1)
            return "Down-Left";
        if (x == -1 && z == 0)
            return "Left";
    ...
    string GetColorHex(LogType type)
    {
        switch (type)
        {
            ...
            // Removed the case for LogType.Info as it does not exist in Unity's LogType enum
            default:
